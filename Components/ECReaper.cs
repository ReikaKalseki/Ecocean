...
namespace ReikaKalseki.Ecocean
{
		internal class ECReaper : MonoBehaviour {
			...
			void Update() {
				...
				float f = Mathf.Clamp01((distq-15625F)/(62500F));
				float glow = 200000*f*f*f;
				...
				if (isInVehicleWithSonar()) {
					...
					if ((isRoaring(roar1) || isRoaring(roar2)) && Mathf.Sin(DayNightCycle.main.timePassedAsFloat*0.8F+gameObject.GetInstanceID()) > 0) { //sin is just because the sound is always "playing"
