...
namespace ReikaKalseki.Ecocean {
	public class BaseCellEnviroHandler : MonoBehaviour {
		...
		void Update() {
			if (!cell)
				cell = this.GetComponent<BaseCell>();
		...
		public void computeBaseCell() {
			UseableDiveHatch[] hatches = this.GetComponentsInChildren<UseableDiveHatch>();
			...
			hasMoonpool = (bool)this.GetComponentInChildren<VehicleDockingBay>();
			if (hasMoonpool) {
				boundingBoxPlanktonSpawns.Clear();
