...
namespace ReikaKalseki.Ecocean {
	...
	public class PiezoCrystal : Spawnable {
		...
		public override GameObject GetGameObject() {
			...
			foreach (Vector3 ang in angs) {
				...
				go.removeComponent<LargeWorldEntity>();
				go.removeComponent<EntityTag>();
				go.removeComponent<WorldForces>();
			...
			dr.onDrilled += (d) => {
				d.GetComponent<PiezoCrystalTag>().onDrilled(d.transform.position);
			};/*
	...
	public class PiezoCrystalTag : MonoBehaviour, ReactsOnDrilled {
		...
		void Update() {
			...
			if (!drill)
				drill = this.GetComponent<Drillable>();
			...
			if (sources.Count == 0) {
				MeshRenderer[] rs = this.GetComponentsInChildren<MeshRenderer>();
			...
			if (drill.drillingExo && !exploded)
				this.onDrilled(transform.position);
		}
