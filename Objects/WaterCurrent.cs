...
namespace ReikaKalseki.Ecocean {
	...
	public abstract class WaterCurrentBase<T> : Spawnable where T : WaterCurrentTag {
		...
		public override GameObject GetGameObject() {
			GameObject world = ObjectUtil.createWorldObject("42b38968-bd3a-4bfd-9d93-17078d161b29").setName(ClassID+"[Clone]");
	...
	public abstract class WaterCurrentTag : MonoBehaviour {
		...
		void Update() {
			if (!current)
				current = this.GetComponent<Current>();
			if (!render)
		...
		public float getCurrentStrength(Vector3 pos) {
			...
			if (this.isInCylinder(pos, pt1, pt2))
				return 1;
