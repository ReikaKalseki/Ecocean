...
namespace ReikaKalseki.Ecocean {
	...
	public class MushroomVaseStrand : BasicCustomPlant, CustomHarvestBehavior {
		...
		public MushroomVaseStrand(XMLLocale.LocaleEntry e) : base(e, new FloraPrefabFetch(VanillaFlora.REDWORT), "8bc4f11e-17b9-447e-be0c-2fbe324e64f5", "Tendrils") {
			...
			OnFinishedPatching += () => {
				addPDAEntry(e.pda, 2F, e.getField<string>("header"));
		...
		public class MushroomVaseStrandTag : MonoBehaviour, IHandTarget {
			...
			void Start() {
				...
				if (grown) {
	    			...
	    			if (bc) {
	    				...
	    				GameObject cdr = ObjectUtil.getChildObject(gameObject, "Capsule");
	    				if (cdr) {
	    			...
	    			if (!planktonClearingArea) {
	    				GameObject go = ObjectUtil.getChildObject(gameObject, PLANKTON_AOE);
	    				if (!go) {
	    				...
	    				planktonClearingMgr.onClearTick += (pc, amt) => { resourceGenerationProgress = Mathf.Min(1, resourceGenerationProgress+amt/HARVEST_CYCLE_TIME*getResourceFilterEfficiency()); };
	    			}
	    			...
	    			if (!interactTrigger) {
	    				GameObject go = gameObject;/*ObjectUtil.getChildObject(gameObject, INTERACT_TRIGGER);
	    				if (!go) {
			...
			private void pickResource() {
				if (resourceGenerationProgress >= 1) {
					InventoryUtil.addItem(MushroomVaseStrand.filterDrops.getRandomEntry());
